...
CREATE INDEX idx_practice_sessions_student_status ON practice_sessions(student_id, status);

CREATE INDEX idx_practice_questions_session_id ON practice_questions(session_id);
CREATE INDEX idx_practice_questions_quiz_id ON practice_questions(quiz_id);
CREATE INDEX idx_practice_questions_knowledge_point ON practice_questions(knowledge_point_id);
CREATE INDEX idx_practice_questions_session_unanswered ON practice_questions(session_id, student_answer) WHERE student_answer IS NULL;
CREATE INDEX idx_practice_questions_session_number ON practice_questions(session_id, question_number);

-- Create practice_statistics view for quick analytics
CREATE OR REPLACE VIEW practice_statistics_view AS
SELECT 
    ps.student_id,
