...
export class KnowledgePointService {
  ...
  async getHierarchyBySubject(subjectId: string): Promise<any> {
    ...
    knowledgePoints.forEach(kp => {
      ...
      if (!hierarchy[kp.volume].units[kp.unit].lessons[kp.lesson].sections[kp.sub]) {
        hierarchy[kp.volume].units[kp.unit].lessons[kp.lesson].sections[kp.sub] = {
          topics: []
        };
      }
      
      hierarchy[kp.volume].units[kp.unit].lessons[kp.lesson].sections[kp.sub].topics.push({
        id: kp.id,
        topic: kp.topic
      });
