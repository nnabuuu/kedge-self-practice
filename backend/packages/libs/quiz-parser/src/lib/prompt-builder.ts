...
填空题提示词（hints）说明：
- **优先使用原文提示**：如果高亮内容附近有任何形式的提示文本，直接提取使用
  * 括号形式："<highlight>共产党和工人党情报局</highlight>（机构）" → hints: ["机构"]
  * 其他可能的提示格式：【】、「」、注释、标注等
  * 原文提示优先级最高，智能识别并提取，不要自己再创造新的提示词
- 如果原文没有任何提示，为每个空格提供一个简洁的提示词（2-4个字），描述答案的类型
- 提示词应该帮助学生理解需要填写什么类型的内容，但不能透露答案
- 如果答案从上下文很明显，可以不提供提示（使用null）
- 自动生成提示词参考（仅在原文无提示时使用）：
  * 时间类：年份、朝代、世纪、时期、年代
  * 人物类：人名、皇帝、领袖、思想家、将领
  * 地理类：地名、国家、都城、地区、关隘
  * 事件类：战争、事件、条约、改革、起义
...
- **顺序无关的并列空格（重要）**：
  * 当题目中相邻空格之间有"和"、"与"、"以及"等并列连词时，说明这些空格顺序可互换
  * 例如："北宋和北方的____、____政权" 答案：["辽", "西夏"]
  * 这种情况下，必须为每个空格添加另一个空格的答案作为替代答案
  * 格式：[["西夏"], ["辽"]] - 第0个空格可以填"西夏"，第1个空格可以填"辽"
  * 这样学生填写"西夏、辽"或"辽、西夏"都能被正确判定
  * 检测规则：如果空格之间的文字只有"、"或"和"或"与"或"以及"等连词，则认为是并列关系
...
填空题示例3（使用原文提示）：
输入：
- "1947年3月，"杜鲁门主义"演说发表。为应对美国挑战，苏联等东欧国家成立共产党和工人党情报局（机构）。" (高亮: 共产党和工人党情报局（机构）)
...
填空题示例4（顺序无关的并列空格）：
输入：
- "北宋和北方的辽、西夏政权分别达成和议" (高亮: 辽, 西夏)

输出：
{
  "items": [{
    "type": "fill-in-the-blank",
    "question": "北宋和北方的____、____政权分别达成和议",
    "options": [],
    "answer": ["辽", "西夏"],
    "hints": ["政权", "政权"],
    "alternative_answers": [["西夏"], ["辽"]]
  }]
}

说明：因为两个空格之间只有"、"，属于并列关系，所以：
- 第0个空格的替代答案包含"西夏"（第1个空格的答案）
- 第1个空格的替代答案包含"辽"（第0个空格的答案）
- 这样学生无论填"辽、西夏"还是"西夏、辽"都能正确`;
  }
