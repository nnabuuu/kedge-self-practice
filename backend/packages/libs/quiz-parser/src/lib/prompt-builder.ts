...
填空题提示词（hints）说明：
- **优先使用原文提示**：如果高亮内容后有括号提示（如"<highlight>答案</highlight>（提示）"），直接使用该提示词
  * 例如："<highlight>共产党和工人党情报局</highlight>（机构）" → hints: ["机构"]
  * 原文提示优先级最高，不要自己再创造新的提示词
- 如果原文没有提示，为每个空格提供一个简洁的提示词（2-4个字），描述答案的类型
