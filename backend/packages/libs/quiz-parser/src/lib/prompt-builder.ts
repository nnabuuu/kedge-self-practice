...
- **顺序无关的并列空格（重要）**：
  * 对于多空格填空题，智能判断各空格之间是否存在顺序要求
  * 判断标准（综合考虑，不只看连词）：
    - 如果答案是平等的并列关系，且顺序不影响语义，则可互换
    - 例如："北宋和北方的____、____政权" → "辽"和"西夏"地位平等，可互换
...
说明：判断"辽"和"西夏"是平等并列的两个政权，顺序可互换，所以：
- 第0个空格的替代答案包含"西夏"（第1个空格的答案）
- 第1个空格的替代答案包含"辽"（第0个空格的答案）
- 这样学生无论填"辽、西夏"还是"西夏、辽"都能正确

反例（不可互换的情况）：
"____的____建立了元朝" 答案：["忽必烈", "蒙古族"]
这里不应添加互换的替代答案，因为"忽必烈的蒙古族"语义不通`;
  }
