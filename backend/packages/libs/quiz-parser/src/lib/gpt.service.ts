...
export class GptService {
  ...
  async extractQuizItems(paragraphs: GptParagraphBlock[]): Promise<QuizItem[]> {
    ...
    4. 题目内容可能跨paragraph存在，请将这些内容一并包括在题目中，不要漏掉。尤其注意image placeholder，某些paragraph顺序可能混乱，请根据语义引入并保持正确的文本顺序。例如：题干中表示"下图..."，则说明附近的一个或多个{{image:uuid}}应该被纳入该题目。
    5. 不要创造新的选项。仅在原文中明确列出可供选择的选项(如"1."、"①"、"A. "等)时，才可生成选择题。
    6. 若原文中未明确列出多个选项，但包含某个高亮词汇，请将其作为"填空题"处理，"填空题"可以有多个高亮指示的空白处。
    7. 若原文中出现大段答案结果且仅有一处高亮，请将其作为"主观题"处理
    8. 如果题干/选项/答案以数字或编号开头(如"1."、"①"、"A. "等)，请将这些部分从题干中去除，仅保留纯粹的题干/选项/答案。
    9. 返回的 JSON 格式必须完全符合以下结构：
    每道题返回：
